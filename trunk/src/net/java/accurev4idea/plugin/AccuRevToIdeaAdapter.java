...
public class AccuRevToIdeaAdapter implements ProjectComponent {
    ...
    public DiffContent getEditorDiffContent(VirtualFile virtualFile) {
        ...
        Charset charset = Charset.forName(SystemUtils.FILE_ENCODING); //FIXED
        return new BinaryContent(doc.getText().getBytes(), charset, FileTypeManager.getInstance().getFileTypeByFile(virtualFile));
    }
    ...
    private DiffContent getDiffContent(VirtualFile virtualFile, Command command) {
        ...
        Charset charset = Charset.forName(SystemUtils.FILE_ENCODING); //FIXED
        BinaryContent diffContent = new BinaryContent(fileContents, charset, fileType);
        return diffContent;
    }
